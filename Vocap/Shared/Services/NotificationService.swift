...
final class NotificationService: ObservableObject {
    ...
    func scheduleTestNotification(word: WordDTO, inSeconds seconds: TimeInterval = 5) async throws {
        guard isAuthorized else {
            throw NotificationError.notAuthorized
        }
    ...
    func cancelNotifications(for wordId: UUID) async {
        let pending = await center.pendingNotificationRequests()
        let toRemove = pending
            .filter { $0.identifier.contains(wordId.uuidString) }
            .map { $0.identifier }
